...
## Clearing the Table: FP's Reputation
...
We care, frankly, because *we're not that smart*. Let's think about some of the kinds of things we're doing with, say, restaurant software: clients, with locations, building baskets, composed of products with options and modifiers, which have a set of rules for what combinations are allowed both of products and of their elements as making up a basket, which turn into orders, which have associated payment schemes (sometimes a lot of them), which generate data to send to a POS as well as summaries for the customer who ordered it, and so on. There are a *lot* of moving pieces there. I'm sure a missed some non-trivial pieces, too. And if all of that is *stateful*, that's a lot of state to hold in your head.
...
What about *pure functions*? Pure functions are functions with *no effects*. The input directly translates to the output, every time. The examples we looked at just a moment ago with `map` are all pure functions (and it's a really weird antipattern to use effectful functions with `map`!). Here are a few more super simple examples:
...
We have fewer things to keep in our heads when we look at any given pure function, because we don't have to worry at all about whether something it touches has been changed by another function or not. We have inputs. We transform them into outputs. That's it. Compare these two things:
...
You're probably wondering how in the world this can work (and also how you avoid it being incredibly computationally expensive).

---

## A Worked Example

Let's get down to a real example of these ideas. Most everyone here should be familiar with the burger test we use as part of our hiring process. For anyone who isn't, the idea is, you get a list of burger orders which looks like this:
...
Unfortunately, given the amount of data we're dealing with, that's prohibitively expensive. We end up spending a *lot* of time allocating objects and garbage-collecting them. As a result, it's thousands, if not millions, of times slower. Running it on my 4GHz iMac, the in-place version takes less than 40ms. Doing it the strictly pure way—returning copies every time—takes ~53s. And if you profile it, almost all of that time is spent in `assign` (52.95s).
...
And... two of these methods *are* not doing that, but are modifying an array in place instead. Who knows which they are? (`sort` and `forEach`).
...
One way around this is to use Lodash's `_.chain` method. That would have let use write it like this:
