...
Over time, this becomes a system of notes, which can reference and build on each other. Your *collection* of notes transforms into a *network*, where each note is a node in a web of other notes. If you build up the web carefully, you can follow its lines in novel directions over time, which is an aid to memory and learning—but, more importantly in my opinion, also to *insight*. Surprising connections between ideas can emerge over time. Those connections in turn can lead to whole new lines of questions and ideas.
